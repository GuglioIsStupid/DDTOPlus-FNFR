...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    */
    ...
    float red 	=   Texel(	texture, 	vec2(uv.x + xOffset -0.01*rgbOffsetOpt,uv.y+yOffset)).r+staticVal;
    float green = 	Texel(	texture, 	vec2(uv.x + xOffset,	  uv.y+yOffset)).g+staticVal;
    float blue 	=	Texel(	texture, 	vec2(uv.x + xOffset +0.01*rgbOffsetOpt,uv.y+yOffset)).b+staticVal;
    float flAlpha = 	Texel(	texture, 	vec2(uv.x + xOffset,	  uv.y+yOffset)).a+staticVal;

    return vec4(red,green,blue,flAlpha);
}
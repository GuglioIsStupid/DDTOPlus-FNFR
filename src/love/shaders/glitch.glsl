extern number time;
extern number prob;
extern number intensityChromatic;
...
void glitchSwap(inout vec2 p) {
    ...
    seed = glitchSeed(glitchCoord(pp, groupSize), speed);
    ...
    swapBlocks(p, groupSize, subGrid, blockSize, (seed.seed + 1.), apply2);
    swapBlocks(p, groupSize, subGrid, blockSize, (seed.seed + 2.), apply2);
    swapBlocks(p, groupSize, subGrid, blockSize, (seed.seed + 3.), apply2);
    swapBlocks(p, groupSize, subGrid, blockSize, (seed.seed + 4.), apply2);
    swapBlocks(p, groupSize, subGrid, blockSize, (seed.seed + 5.), apply2);
...
vec4 transverseChromatic(vec2 p, Image texture) {
    vec2 destCoord = p;
    ...
    vec3 accumalator = vec3(0);
    mat3x2 offsets = mat3x2(0);
    for (int i = 0; i < sampleCount; i++) {
        accumalator.r += Texel(texture, destCoord + offsets[0]).r;
        accumalator.g += Texel(texture, destCoord + offsets[1]).g;
        accumalator.b += Texel(texture, destCoord + offsets[2]).b;
        offsets -= increments;
    }
    // get newColor with the current alpha channel
    vec4 newColor = vec4(accumalator * inverseSampleCount, Texel(texture, destCoord).a);
    return newColor;
}

vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
    // get color WITH the alpha channel
    vec4 pixel = Texel(texture, texture_coords);
    vec2 p = texture_coords;
    glitchSwap(p);
    glitchStatic(p);
    // glitchTime(p, time);
    glitchColor(p, pixel.rgb);

    pixel = transverseChromatic(p, texture);
